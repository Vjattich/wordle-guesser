...
const toElement = function (e) {
    let positionClassPosition = 1,
        ...
        isEmpty = e.value == '' || e.value == null;

    return {
        value: e,
        char: isEmpty ? e.value : e.value.toLowerCase(),
        isEmpty: isEmpty,
        isNotEmpty: !isEmpty,
...
const toEvent = function (e) {
    ...
    return {
        ...
        isRightPress: indexOfKeyCode === 2,
        isDownPress: indexOfKeyCode === 3,
    }
};
...
const getSibling = function (self, isNext) {
    ...
    if (element.isNotInput) {
        let main = element.value.parentNode;
        if (main.nextElementSibling) {
            return main.nextElementSibling.childNodes[0];
        }
    }
...
const getInputSibling = function (self, isPrev) {

    if (!self) {
        return;
    }

    let element = toElement(self),
        prevParentNode = isPrev ? element.value.parentNode.previousElementSibling : element.value.parentNode.nextElementSibling;

    if (!prevParentNode || prevParentNode.tagName === 'LABEL') {
        return self;
    }

    return prevParentNode.childNodes[element.position - 1];
};

const upInput = function (self) {
    return getInputSibling(self, true)
};
...
const addInput = function (e) {
    ...
    clone.childNodes.forEach(s => {
        ...
        s.classList.replace('yellow', 'gray')
    })
    ...
    return Array.from(inputs)
        ...
        .reduce((acc, e) => {
            ...
            let obj = {
                ...
                position: e.position,
                toIdString: function () {
                    return color.isWhite ? this.char + this.position : this.char;
                },
            };
            ...
            return acc;
        }, {exact: new WorkingSet(), contains: new WorkingSet(), notContains: new WorkingSet()});
}

const makePattern = function (notContainsChar, exactChar, containsChar) {

    let exact = exactChar.values().reduce(function (acc, letter) {
        acc[letter.position] = letter.char;
        return acc
    }, []);

    let contains = containsChar.values().reduce(function (acc, letter) {
            let accElement = acc[letter.position];
            acc[letter.position] = accElement ? accElement + letter.char : letter.char;
            return acc
        }, []),
        containsLetters = Array.from(
            contains
    ...
    let letters = notContainsChar.values().reduce(function (acc, letter) {

        if (containsLetters.indexOf(letter.char) > -1) {
            return acc;
        }
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        let contain = contains[i];

        if (contain) {
            if (patternElement) {
                if (patternElement.indexOf(contain) === -1) {
                    patternElement = patternElement + contain;
                }
            } else {
                patternElement = contain;
            }
        }
    ...
    let map = containsLetters
        .map(function (l) {
            return '(?=.*' + l + ')'
        })
        .join('');

    return new RegExp(map + pat.join(''));
};


const filterWords = function (chars) {
    let pattern = makePattern(chars.notContains, chars.exact, chars.contains);
    return words
        .filter(function (word) {return pattern.test(word)});
};
...
const changeWord = function (words) {
    document.getElementById('words').innerHTML = words.map(w => '<div>' + w + '</div>').join('');
};
